...
static constexpr wchar_t FINNISH_CONSONANTS[] = {
    ...
    L'p', L'q', L'r', L's', L't', L'v', L'w', L'x', L'z', L'B', L'C',
    L'D', L'F', L'G', L'H', L'J', L'K', L'L', L'M', L'N', L'P', L'Q',
    L'R', L'S', L'T', L'V', L'W', L'X', L'Z', 0};
...
static const wchar_t PORTUGUESE_VOWELS[] = {
    97, 101, 105, 111, 117,  0xE1, 0xE9, 0xED, 0xF3, 0xFA, 0xE2, 0xEA, 0xF4, 65,
    69, 73,  79,  85,  0xC1, 0xC9, 0xCD, 0xD3, 0xDA, 0xC2, 0xCA, 0xD4, 0};
...
/** @brief Converts a full-width number/English letter/various symbols
   ...
   full-width.*/
[[nodiscard]] inline constexpr wchar_t full_width_to_narrow(
...
/** @brief The base class for language-specific stemmers.
    ...
    This is to say, if your `basic_string` has a custom character traits or
   allocator, then just specify it in your template argument to the stemmer.
...
class stem {
   ...
   protected:
    ...
    void find_r1(const string_typeT& text, const wchar_t* vowel_list) noexcept {
    ...
    /// @param text The string to review.
    /// @param vowel_list The list of vowels by the stemmer's language.
    void find_r2(const string_typeT& text, const wchar_t* vowel_list) noexcept {
        size_t start = 0;
    ...
    /* @brief Finds the start of RV (French stemmer).
       @param text The string to review.
       @param vowel_list The list of vowels by the stemmer's language.
       @note If the word begins with two vowels, RV is the region after the
       ...
       be found. (Exceptionally, par, col or tap, at the beginning of a word is
    ...
    [[nodiscard]] constexpr bool is_apostrophe(
        const wchar_t& ch) const noexcept {
        return (ch == 39)
                   ?  // '
                   true
                   : (ch == 146)
    ...
    /// @brief Removes possessive suffix (apostrophe and "'s") from the end of a
    /// string.
    /// @param[in,out] text The string to trim.
    void remove_possessive_suffix(string_typeT& text) const {
        if (text.length() >= 2 && is_apostrophe(text[text.length() - 2]) &&
            stem<string_typeT>::is_either(text.back(),
                                          common_lang_constants::LOWER_S,
                                          common_lang_constants::UPPER_S)) {
            text.erase(text.length() - 2);
        }

        while (text.length() >= 1 && is_apostrophe(text.back())) {
            text.pop_back();
        }
    }
    ...
    /// @param text The string being reviewed.
    /// @param suffix1L The lowercased version of the first character of the
    /// suffix.
    /// @param suffix1U The uppercased version of the first character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        const wchar_t suffix1U) noexcept {
        if (text.length() < 1) {
            return false;
        }
        return stem<string_typeT>::is_either(text[text.length() - 1], suffix1L,
                                             suffix1U);
    }
    ...
    /// @param suffix1U The uppercased version of the first character of the
    /// suffix.
    /// @param suffix2L The lowercased version of the second character of the
    /// suffix.
    /// @param suffix2U The uppercased version of the second character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        const wchar_t suffix1U, const wchar_t suffix2L,
        const wchar_t suffix2U) noexcept {
        if (text.length() < 2) {
            return false;
        }
        return stem<string_typeT>::is_either(text[text.length() - 2], suffix1L,
                                             suffix1U) &&
               stem<string_typeT>::is_either(text[text.length() - 1], suffix2L,
                                             suffix2U);
    }
    ...
    /// @param suffix2U The uppercased version of the second character of the
    /// suffix.
    /// @param suffix3L The lowercased version of the third character of the
    /// suffix.
    /// @param suffix3U The uppercased version of the third character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        ...
        return stem<string_typeT>::is_either(text[text.length() - 3], suffix1L,
                                             suffix1U) &&
               stem<string_typeT>::is_either(text[text.length() - 2], suffix2L,
                                             suffix2U) &&
               stem<string_typeT>::is_either(text[text.length() - 1], suffix3L,
                                             suffix3U);
    }
    ...
    /// @param suffix3U The uppercased version of the third character of the
    /// suffix.
    /// @param suffix4L The lowercased version of the fourth character of the
    /// suffix.
    /// @param suffix4U The uppercased version of the fourth character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        ...
        const wchar_t suffix4U) noexcept {
        if (text.length() < 4) {
            return false;
        }
        return stem<string_typeT>::is_either(text[text.length() - 4], suffix1L,
                                             suffix1U) &&
               stem<string_typeT>::is_either(text[text.length() - 3], suffix2L,
                                             suffix2U) &&
               stem<string_typeT>::is_either(text[text.length() - 2], suffix3L,
                                             suffix3U) &&
               stem<string_typeT>::is_either(text[text.length() - 1], suffix4L,
                                             suffix4U);
    }
    ...
    /// @param suffix4U The uppercased version of the fourth character of the
    /// suffix.
    /// @param suffix5L The lowercased version of the fifth character of the
    /// suffix.
    /// @param suffix5U The uppercased version of the fifth character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        ...
        const wchar_t suffix5U) noexcept {
        if (text.length() < 5) {
            return false;
        }
        return stem<string_typeT>::is_either(text[text.length() - 5], suffix1L,
                                             suffix1U) &&
               stem<string_typeT>::is_either(text[text.length() - 4], suffix2L,
                                             suffix2U) &&
               stem<string_typeT>::is_either(text[text.length() - 3], suffix3L,
                                             suffix3U) &&
               stem<string_typeT>::is_either(text[text.length() - 2], suffix4L,
                                             suffix4U) &&
               stem<string_typeT>::is_either(text[text.length() - 1], suffix5L,
                                             suffix5U);
    }
    ...
    /// @param suffix5U The uppercased version of the fifth character of the
    /// suffix.
    /// @param suffix6L The lowercased version of the sixth character of the
    /// suffix.
    /// @param suffix6U The uppercased version of the sixth character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        ...
        return stem<string_typeT>::is_either(text[text.length() - 6], suffix1L,
                                             suffix1U) &&
               stem<string_typeT>::is_either(text[text.length() - 5], suffix2L,
                                             suffix2U) &&
               stem<string_typeT>::is_either(text[text.length() - 4], suffix3L,
                                             suffix3U) &&
               stem<string_typeT>::is_either(text[text.length() - 3], suffix4L,
                                             suffix4U) &&
               stem<string_typeT>::is_either(text[text.length() - 2], suffix5L,
                                             suffix5U) &&
               stem<string_typeT>::is_either(text[text.length() - 1], suffix6L,
                                             suffix6U);
    }
    ...
    /// @param suffix6U The uppercased version of the sixth character of the
    /// suffix.
    /// @param suffix7L The lowercased version of the seventh character of the
    /// suffix.
    /// @param suffix7U The uppercased version of the seventh character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        ...
        const wchar_t suffix7U) noexcept {
        if (text.length() < 7) {
            return false;
        }
        return stem<string_typeT>::is_either(text[text.length() - 7], suffix1L,
                                             suffix1U) &&
               stem<string_typeT>::is_either(text[text.length() - 6], suffix2L,
                                             suffix2U) &&
               stem<string_typeT>::is_either(text[text.length() - 5], suffix3L,
                                             suffix3U) &&
               stem<string_typeT>::is_either(text[text.length() - 4], suffix4L,
                                             suffix4U) &&
               stem<string_typeT>::is_either(text[text.length() - 3], suffix5L,
                                             suffix5U) &&
               stem<string_typeT>::is_either(text[text.length() - 2], suffix6L,
                                             suffix6U) &&
               stem<string_typeT>::is_either(text[text.length() - 1], suffix7L,
                                             suffix7U);
    }
    ...
    /// @param suffix7U The uppercased version of the seventh character of the
    /// suffix.
    /// @param suffix8L The lowercased version of the eighth character of the
    /// suffix.
    /// @param suffix8U The uppercased version of the eighth character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        ...
        const wchar_t suffix8U) noexcept {
        if (text.length() < 8) {
            return false;
        }
        return stem<string_typeT>::is_either(text[text.length() - 8], suffix1L,
                                             suffix1U) &&
               stem<string_typeT>::is_either(text[text.length() - 7], suffix2L,
                                             suffix2U) &&
               stem<string_typeT>::is_either(text[text.length() - 6], suffix3L,
                                             suffix3U) &&
               stem<string_typeT>::is_either(text[text.length() - 5], suffix4L,
                                             suffix4U) &&
               stem<string_typeT>::is_either(text[text.length() - 4], suffix5L,
                                             suffix5U) &&
               stem<string_typeT>::is_either(text[text.length() - 3], suffix6L,
                                             suffix6U) &&
               stem<string_typeT>::is_either(text[text.length() - 2], suffix7L,
                                             suffix7U) &&
               stem<string_typeT>::is_either(text[text.length() - 1], suffix8L,
                                             suffix8U);
    }
    ...
    /// @param suffix8U The uppercased version of the eighth character of the
    /// suffix.
    /// @param suffix9L The lowercased version of the ninth character of the
    /// suffix.
    /// @param suffix9U The uppercased version of the ninth character of the
    /// suffix.
    /// @returns @c true if characters match suffix.
    [[nodiscard]] inline static bool is_suffix(
        const string_typeT& text, const wchar_t suffix1L,
        ...
        return stem<string_typeT>::is_either(text[text.length() - 9], suffix1L,
                                             suffix1U) &&
               stem<string_typeT>::is_either(text[text.length() - 8], suffix2L,
                                             suffix2U) &&
               stem<string_typeT>::is_either(text[text.length() - 7], suffix3L,
                                             suffix3U) &&
               stem<string_typeT>::is_either(text[text.length() - 6], suffix4L,
                                             suffix4U) &&
               stem<string_typeT>::is_either(text[text.length() - 5], suffix5L,
                                             suffix5U) &&
               stem<string_typeT>::is_either(text[text.length() - 4], suffix6L,
                                             suffix6U) &&
               stem<string_typeT>::is_either(text[text.length() - 3], suffix7L,
                                             suffix7U) &&
               stem<string_typeT>::is_either(text[text.length() - 2], suffix8L,
                                             suffix8U) &&
               stem<string_typeT>::is_either(text[text.length() - 1], suffix9L,
                                             suffix9U);
    }
    ...
    [[nodiscard]] inline static bool is_partial_suffix(
        ...
        return (stem<string_typeT>::is_either(text[start_index], suffix1L,
                                              suffix1U) &&
                stem<string_typeT>::is_either(text[start_index + 1], suffix2L,
                                              suffix2U));
    }
    ...
    [[nodiscard]] inline static bool is_partial_suffix(
        const string_typeT& text, const size_t start_index,
        const wchar_t suffix1L, const wchar_t suffix1U, const wchar_t suffix2L,
        const wchar_t suffix2U, const wchar_t suffix3L,
        const wchar_t suffix3U) noexcept {
        ...
        return (stem<string_typeT>::is_either(text[start_index], suffix1L,
                ...
                stem<string_typeT>::is_either(text[start_index + 1], suffix2L,
                                              suffix2U) &&
                stem<string_typeT>::is_either(text[start_index + 2], suffix3L,
                                              suffix3U));
    }
    ...
    [[nodiscard]] inline bool is_suffix_in_rv(const string_typeT& text,
                                              ...
                                              const wchar_t suffix2L,
                                              const wchar_t suffix2U) noexcept {
        if (text.length() < 2) {
            return false;
        }
        return ((stem<string_typeT>::is_either(text[text.length() - 2],
                                               suffix1L, suffix1U) &&
    ...
    /// suffix.
    /// @returns @c true if suffix is in RV.
    [[nodiscard]] inline bool is_suffix_in_rv(
        const string_typeT& text, const wchar_t suffix1L,
        const wchar_t suffix1U, const wchar_t suffix2L, const wchar_t suffix2U,
        const wchar_t suffix3L, const wchar_t suffix3U) noexcept {
        if (text.length() < 3) {
        ...
        return ((stem<string_typeT>::is_either(text[text.length() - 3],
                                               suffix1L, suffix1U) &&
    ...
    [[nodiscard]] inline bool is_suffix_in_rv(
        ...
        return ((stem<string_typeT>::is_either(text[text.length() - 4],
                                               suffix1L, suffix1U) &&
    ...
    [[nodiscard]] inline bool is_suffix_in_rv(
        ...
        return ((stem<string_typeT>::is_either(text[text.length() - 5],
                                               suffix1L, suffix1U) &&
    ...
    [[nodiscard]] inline bool is_suffix_in_rv(
        ...
        return ((stem<string_typeT>::is_either(text[text.length() - 6],
                                               suffix1L, suffix1U) &&
    ...
    [[nodiscard]] inline bool is_suffix_in_rv(
        ...
        return ((stem<string_typeT>::is_either(text[text.length() - 7],
                                               suffix1L, suffix1U) &&
    ...
    [[nodiscard]] inline bool is_suffix_in_rv(
        ...
        return ((stem<string_typeT>::is_either(text[text.length() - 8],
                                               suffix1L, suffix1U) &&
    ...
    /// suffix.
    /// @returns @c true if suffix is in R1.
    [[nodiscard]] inline bool is_suffix_in_r1(
        const string_typeT& text, const wchar_t suffix1L,
        const wchar_t suffix1U, const wchar_t suffix2L, const wchar_t suffix2U,
        const wchar_t suffix3L, const wchar_t suffix3U) noexcept {
        if (text.length() < 3) {
    ...
    /// suffix.
    /// @returns @c true if suffix is in R2.
    [[nodiscard]] inline bool is_suffix_in_r2(
        const string_typeT& text, const wchar_t suffix1L,
        const wchar_t suffix1U, const wchar_t suffix2L, const wchar_t suffix2U,
        const wchar_t suffix3L, const wchar_t suffix3U) noexcept {
        if (text.length() < 3) {
    ...
    [[nodiscard]] inline bool is_suffix_in_r2(
        ...
        const wchar_t suffix6U) noexcept {
        if (text.length() < 6) {
            return false;
        }
        return ((stem<string_typeT>::is_either(text[text.length() - 6],
    ...
    inline bool delete_if_is_in_r1(string_typeT& text, const wchar_t suffix1L,
                                   const wchar_t suffix1U,
        ...
        if (stem<string_typeT>::is_either(text[text.length() - 1], suffix1L,
                                          suffix1U)) {
    ...
    inline bool delete_if_is_in_r1(string_typeT& text, const wchar_t suffix1L,
        ...
        if (stem<string_typeT>::is_either(text[text.length() - 2], suffix1L,
                                          suffix1U) &&
            stem<string_typeT>::is_either(text[text.length() - 1], suffix2L,
                                          suffix2U)) {
    ...
    inline bool delete_if_is_in_r1(
        ...
        if (stem<string_typeT>::is_either(text[text.length() - 3], suffix1L,
                                          suffix1U) &&
            stem<string_typeT>::is_either(text[text.length() - 2], suffix2L,
                                          suffix2U) &&
            stem<string_typeT>::is_either(text[text.length() - 1], suffix3L,
    ...
    inline bool delete_if_is_in_r1(
        ...
        if (stem<string_typeT>::is_either(text[text.length() - 4], suffix1L,
                                          suffix1U) &&
            stem<string_typeT>::is_either(text[text.length() - 3], suffix2L,
                                          suffix2U) &&
            stem<string_typeT>::is_either(text[text.length() - 2], suffix3L,
    ...
    inline bool delete_if_is_in_r1(
        ...
        if (stem<string_typeT>::is_either(text[text.length() - 5], suffix1L,
                                          suffix1U) &&
            stem<string_typeT>::is_either(text[text.length() - 4], suffix2L,
                                          suffix2U) &&
            stem<string_typeT>::is_either(text[text.length() - 3], suffix3L,
    ...
    inline bool delete_if_is_in_r1(
        ...
        if (stem<string_typeT>::is_either(text[text.length() - 6], suffix1L,
                                          suffix1U) &&
            stem<string_typeT>::is_either(text[text.length() - 5], suffix2L,
                                          suffix2U) &&
            stem<string_typeT>::is_either(text[text.length() - 4], suffix3L,
    ...
    inline bool delete_if_is_in_r1(
        ...
        if (stem<string_typeT>::is_either(text[text.length() - 7], suffix1L,
                                          suffix1U) &&
            stem<string_typeT>::is_either(text[text.length() - 6], suffix2L,
                                          suffix2U) &&
            stem<string_typeT>::is_either(text[text.length() - 5], suffix3L,
    ...
    /// @param success_on_find Return true if found, but not deleted.
    /// @returns @c true if characters match suffix and are deleted.
    inline bool delete_if_is_in_r2(string_typeT& text, const wchar_t suffix1L,
                                   const wchar_t suffix1U,
                                   const bool success_on_find = true) {
        if (text.length() < 1) {
            return false;
    ...
    inline bool delete_if_is_in_r2(
        ...
        if (stem<string_typeT>::is_either(text[text.length() - 8], suffix1L,
                                          suffix1U) &&
            stem<string_typeT>::is_either(text[text.length() - 7], suffix2L,
                                          suffix2U) &&
            stem<string_typeT>::is_either(text[text.length() - 6], suffix3L,
    ...
    /// @param success_on_find Return true if found, but not deleted.
    /// @returns @c true if characters match suffix and are deleted.
    inline bool delete_if_is_in_rv(string_typeT& text, const wchar_t suffix1L,
                                   const wchar_t suffix1U,
                                   const bool success_on_find = true) {
        if (text.length() < 1) {
            return false;
    ...
    /// @brief Hashes initial y, y after a vowel, and i between vowels into
    /// hashed character.
    /// @param text The string to update.
    ...
    void hash_dutch_yi(string_typeT& text, const wchar_t* vowel_string) {
        ...
        bool in_vowel_block =
            stem<string_typeT>::is_one_of(text[0], vowel_string);
        ...
        for (i = 1; i < text.length() - 1; ++i) {
            if (in_vowel_block && text[i] == common_lang_constants::LOWER_I &&
                ...
                in_vowel_block = false;
                       ...
                       stem<string_typeT>::is_one_of(text[i + 1],
                                                     vowel_string)) {
                text[i] = UPPER_I_HASH;
                in_vowel_block = false;
            } else if (in_vowel_block &&
    ...
    inline void unhash_dutch_yi(string_typeT& text) {
        std::transform(
            text.cbegin(), text.cend(), text.begin(),
            [](const auto& ch) noexcept {
                return (ch == LOWER_Y_HASH)   ? common_lang_constants::LOWER_Y
                       : (ch == UPPER_Y_HASH) ? common_lang_constants::UPPER_Y
    ...
    by a vowel. u after q is also hashed. For example,\n jouer        -> joUer
        ennuie       ->         ennuIe
        yeux         ->         Yeux
        quand        ->         qUand
    ...
    void hash_french_yui(string_typeT& text, const wchar_t* vowel_string) {
        ...
        if (text[i] == common_lang_constants::LOWER_Y && (i > 0) &&
            stem<string_typeT>::is_one_of(text[i - 1], vowel_string)) {
            text[i] = LOWER_Y_HASH;
        } else if (text[i] == common_lang_constants::UPPER_Y && (i > 0) &&
                   stem<string_typeT>::is_one_of(text[i - 1], vowel_string)) {
            text[i] = UPPER_Y_HASH;
        } else if (text[i] == common_lang_constants::LOWER_U && (i > 0) &&
                   stem<string_typeT>::is_either(
                       text[i - 1], common_lang_constants::LOWER_Q,
                       common_lang_constants::UPPER_Q)) {
            text[i] = LOWER_U_HASH;
        } else if (text[i] == common_lang_constants::UPPER_U && (i > 0) &&
                   stem<string_typeT>::is_either(
                       text[i - 1], common_lang_constants::LOWER_Q,
                       common_lang_constants::UPPER_Q)) {
            text[i] = UPPER_U_HASH;
    ...
    void unhash_french_yui(string_typeT& text) {
        stem<string_typeT>::replace_all(text, LOWER_Y_HASH,
                                        common_lang_constants::LOWER_Y);
        stem<string_typeT>::replace_all(text, UPPER_Y_HASH,
                                        common_lang_constants::UPPER_Y);
        stem<string_typeT>::replace_all(text, LOWER_U_HASH,
                                        common_lang_constants::LOWER_U);
        stem<string_typeT>::replace_all(text, UPPER_U_HASH,
                                        common_lang_constants::UPPER_U);
        stem<string_typeT>::replace_all(text, LOWER_I_HASH,
                                        common_lang_constants::LOWER_I);
        stem<string_typeT>::replace_all(text, UPPER_I_HASH,
    ...
    void hash_italian_ui(string_typeT& text, const wchar_t* vowel_string) {
        ...
        for (i = 1; i < text.length() - 1; ++i) {
            ...
            if (in_vowel_block &&
                std::find(uiValues.cbegin(), uiValues.cend(), text[i]) !=
                    uiValues.cend() &&
                stem<string_typeT>::is_one_of(text[i + 1], vowel_string)) {
                if (text[i] == common_lang_constants::LOWER_I) {
                    text[i] = LOWER_I_HASH;
        ...
        if (text[i] == common_lang_constants::LOWER_U && (i > 0) &&
            stem<string_typeT>::is_either(text[i - 1],
    ...
    inline void unhash_italian_ui(string_typeT& text) noexcept {
        std::transform(
            text.cbegin(), text.cend(), text.begin(),
            [](const auto& ch) noexcept {
                return (ch == LOWER_I_HASH)   ? common_lang_constants::LOWER_I
                       : (ch == UPPER_I_HASH) ? common_lang_constants::UPPER_I
                       : (ch == LOWER_U_HASH) ? common_lang_constants::LOWER_U
                       : (ch == UPPER_U_HASH) ? common_lang_constants::UPPER_U
                                              : ch;
    ...
    [[nodiscard]] inline size_t get_r1() const noexcept { return m_r1; }
    /// Sets the position of R1.
    /// @param pos The position.
    inline void set_r1(const size_t pos) noexcept { m_r1 = pos; }

    /// @returns The position of R2.
    [[nodiscard]] inline size_t get_r2() const noexcept { return m_r2; }
    /// @brief Sets the position of R2.
    /// @param pos The position.
    inline void set_r2(const size_t pos) { m_r2 = pos; }

    /// @returns The position of RV.
    [[nodiscard]] inline size_t get_rv() const noexcept { return m_rv; }
    /// @brief Sets the position of RV.
    /// @param pos The position.
    inline void set_rv(const size_t pos) { m_rv = pos; }

    /// @brief Resets the positions of R sections to 0.
    inline void reset_r_values() noexcept { m_r1 = m_r2 = m_rv = 0; }
    ...
    [[nodiscard]] inline static constexpr wchar_t tolower_western(
        const wchar_t c) noexcept {
    ...
    [[nodiscard]] inline static constexpr wchar_t is_western_letter(
        const wchar_t ch) noexcept {
        return (
            // A-Z
            (ch >= 0x41 && ch <= 0x5A) ||
    ...
    [[nodiscard]] inline static constexpr bool is_one_of(
        const wchar_t character, const wchar_t* char_string) noexcept {
        if (!char_string) {
            return false;
        }
    ...
    static void replace_all(string_typeT& text,
                            const string_typeT& textToReplace,
    ...
    template <typename T>
    [[nodiscard]] static inline constexpr bool is_either(
        const T value, const T first, const T second) noexcept {
    ...
    template <typename T>
    [[nodiscard]] static inline constexpr bool is_neither(
        const T value, const T first, const T second) noexcept {
        assert(first != second);
        return (value != first && value != second);
    }
