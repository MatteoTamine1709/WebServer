...
std::vector<std::string> split(const std::string& str,
    ...
    while (current.length() > 0) {
        bool found = false;
        for (int i = 0; i < delimiters.size(); i++) {
            if (current.length() >= delimiters[i].length() &&
                current.substr(0, delimiters[i].length()) == delimiters[i]) {
...
std::string getMimeType(const std::string& extension) {
    ...
    if (mimeMap.find(ext) == mimeMap.end()) return "unknown";
    return mimeMap[ext];
}
...
std::string makeEtag(const std::string& filePath) {
    struct tm tm;
    struct stat attrib;
    if (stat(filePath.c_str(), &attrib) == -1) return "";
    std::stringstream ss;
    ss << attrib.st_mtime << "-";
    ss << attrib.st_size << "-";
    ss << attrib.st_ino;
