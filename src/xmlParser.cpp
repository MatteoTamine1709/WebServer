...
void print_node(const xmlpp::Node* node, unsigned int indentation = 0) {
    ...
    if (nodeText &&
        nodeText->is_white_space())  // Let's ignore the indenting - you don't
    ...
    if (!nodeText && !nodeComment &&
        !nodename.empty())  // Let's not say "name: text".
    {
        const auto namespace_prefix = node->get_namespace_prefix();

        std::cout << indent << "Node name = ";
        if (!namespace_prefix.empty()) std::cout << namespace_prefix << ":";
        std::cout << nodename << std::endl;
    } else if (nodeText)  // Let's say when it's text. - e.g. let's say what
                          // that white space is.
    {
    ...
    if (nodeText) {
        std::cout << indent << "text = \"" << nodeText->get_content() << "\""
                  << std::endl;
    } else if (nodeComment) {
        std::cout << indent << "comment = " << nodeComment->get_content()
                  << std::endl;
    } else if (nodeContent) {
        std::cout << indent << "content = " << nodeContent->get_content()
                  << std::endl;
        ...
        if (attribute) {
            ...
            else if (dynamic_cast<const xmlpp::AttributeDeclaration*>(
                         attribute))
...
int main2(int argc, char* argv[]) {
    ...
    std::string filepath;
    if (argc > argi)
        filepath = argv[argi];  // Allow the user to specify a different XML
                                // file to parse.
    else
    ...
    try {
        ...
        if (validate) parser.set_validate();
        ...
        if (parser) {
            // Walk the tree:
            const auto pNode = parser.get_document()
                                   ->get_root_node();  // deleted by DomParser.
            print_node(pNode);
        }
